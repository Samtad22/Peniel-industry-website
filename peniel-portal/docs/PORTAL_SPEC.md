...
- `print_runs` **(internal table)** — order_id, run_date, shift, colours (Pantones printed), sheets_printed (good), sheets_spoiled, crowns_per_sheet, coating, lacquer, oven_temp_c, coil_lot, notes, entered_by: the coat & print line, where tinplate sheets are coated, lacquered and printed before the presses. Good sheets × crowns per sheet = the crowns they will yield. Recorded by production (and admin); customers never read it.
