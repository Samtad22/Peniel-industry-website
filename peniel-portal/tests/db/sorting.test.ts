...
test("quality records sorting reports; other staff read them; the audit log keeps them", async () => {
  await as(quality, async (db) => {
    await db.query(
      `insert into public.sorting_records (inspection_id, sorted_on, sorted_cartons, waste_cartons, reported_by)
       values ($1, '2026-09-09', 15, 1, 'Garedew'), ($1, '2026-09-16', 16, 0, 'Garedew')`,
      [heldId],
    );
    const { rows } = await db.query("select sum(sorted_cartons)::int as sorted, sum(waste_cartons)::int as waste from public.sorting_records where inspection_id = $1", [heldId]);
    assert.deepEqual(rows[0], { sorted: 31, waste: 1 });
    await db.query("commit");
  });
  ...
  await as(sales, async (db) => {
    ...
    assert.equal(
      await errorCode(db.query("insert into public.sorting_records (inspection_id, sorted_cartons, waste_cartons) values ($1, 1, 0)", [heldId])),
      "42501",
    );
  });
});
...
test("a report needs cartons, and whole non-negative numbers", async () => {
  await as(quality, async (db) => {
    assert.equal(
      await errorCode(db.query("insert into public.sorting_records (inspection_id, sorted_cartons, waste_cartons) values ($1, 0, 0)", [heldId])),
      "23514",
    );
  });
  await as(quality, async (db) => {
    assert.equal(
      await errorCode(db.query("insert into public.sorting_records (inspection_id, sorted_cartons, waste_cartons) values ($1, -3, 0)", [heldId])),
      "23514",
    );
  });
});
